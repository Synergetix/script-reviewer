...
def getStats(text,style):
    ...
    submitnotimer = 0 #submits with no timers
    submittime = 0
    wtdc = 0
    nextc = 0
    shortcount = 0
    ...
    if 'submit' in text.lower():  #Problem! does not find submits without timers!
        if not re.search('[0-9]+:[0-9][0-9]',text) and not re.search('[0-9]+ second',text.lower()) and not re.search('[0-9]+ minute',text.lower()) and not 'long' in text.lower() and not 'medium' in text.lower() and not 'short' in text.lower():
			submitnotimer += 1
	return submitnotimer
    ...
    # if '[next' in text.lower():
        # nextc += 1
    # return [submitnotimer,submittime,wtdc,nextc,shortcount,medcount,longcount,nonstandardsubmittime,longsubmittime]
...
def getlessonitemstats(itemfn):
    '''Calculate statistics of the lesson item.'''
    doc = Document(itemfn)
    ...
    for par in doc.paragraphs[7:]:
	    ...
	    submitnotimers += getStats(text,style)
    ...
    return {
	        'missing submit timers': submitnotimers,
